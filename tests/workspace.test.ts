...
describe('proposals and decisions', () => {
  ...
  it('records agent proposals and human decisions with results', async () => {
    ...
    const d1 = await ws.decide(p1.id, 'approved')
  ...
  it('keeps quarantined files out of the report and lists decisions and CLI commands', async () => {
    ...
    expect(report.summary.proposals).toEqual({ pending: 0, approved: 3, rejected: 1 })
    expect(report.reproduce.cli[0]).toMatch(/^ufo inspect --json/)
    const md = ws.reportMarkdown(report)
    expect(md).toContain('## Agent proposals and human decisions')
